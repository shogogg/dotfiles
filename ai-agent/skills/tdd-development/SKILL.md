...
- Use `Read` **only** for workspace files: `STATE.json`, `PLAN.md`, `QC_SUMMARY.md`, `QC_<CATEGORY>.raw`, `QC_<CATEGORY>.exitcode`, `QC_AUTOFIX.md`, `TASK_LIST.txt`
...
- Run `git push` or any remote push command — push is a user-initiated action only
- Run `gh pr create` or create pull requests in any form — PR creation is a user-initiated action only
...
3. If `currentPhase` exists in STATE.json:
   - Read `currentPhaseId` if present (e.g., `"quality-checks"`).
   - Use `AskUserQuestion`:
     - **前回の続きから再開する** — Resume from Phase N (`<currentPhaseId>`)
     - **最初からやり直す** — Start fresh from Phase 0
   - **Resume**: set `<work-dir>` to existing workspace.
     - If `currentPhaseId` is present: find the matching entry in the **Phases** list below by its ID. If its file number differs from `currentPhase` (skill was renumbered), use the ID to identify the correct file. Then skip to that phase.
     - If `currentPhaseId` is absent (legacy STATE.json): fall back to `currentPhase` number.
...
2. [Exploration](phases/1-exploration.md) (`exploration`) — Load past learnings, check profile, explore codebase
3. [Planning](phases/2-planning.md) (`planning`) — Create work plan and test plan based on exploration results
4. [Approval Gate](phases/3-approval-gate.md) (`approval-gate`) — Present plan (including test plan) to user for approval
5. [Implementation](phases/4-implementation.md) (`implementation`) — Implement code following TDD methodology
6. [Quality Checks](phases/5-quality-checks.md) (`quality-checks`) — Run tests, lint, formatting (max 3 retries)
7. [User Review](phases/6-user-review.md) (`user-review`) — Present changes to user for approval
8. [Final Report](phases/7-final-report.md) (`final-report`) — Summarize results and present to user
9. [PR Review Comments](phases/8-pr-review.md) (`pr-review`) — Address PR review comments (optional, user-initiated)
...
- Phase 8→5 return: Reset Phase 5 retry counter, does NOT increase any cycle counter; set `currentPhaseId` to `"quality-checks"`
