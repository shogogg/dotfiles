...
- **Implementation Units format**: Each unit MUST include a `Model` field specifying the AI model to use (haiku/sonnet/opus) based on task complexity. This model will be used by Phase 5 when launching tdd-implementer sub-agents.
...
- **Metadata tracking**: PLAN.md includes a `## Metadata` section with `Planning Method: codex | self` to record which method was used. This is reported in the final report (Phase 9).
...
- **Key Actions**:
  ...
  - **Lightweight plan editing**: Uses `general-purpose` (Sonnet, max_turns: 10) instead of `task-planner` (Opus) for feedback-driven plan updates — faster and more efficient since only minimal edits are needed

### Phase 5: Implementation
- **Executor**: Multiple sub-agents (tdd-implementer, parallel when possible)
- **Model Selection**: Each Implementation Unit in PLAN.md specifies a `Model` field (haiku/sonnet/opus) based on task complexity. Phase 5 reads this field and passes it to tdd-implementer via `Task(model="<value>")`. Interface stub creation uses `haiku` by default.
- **Key Actions**:
...
- **Key Actions**:
  ...
  - Return to Phase 6 (no push — full quality loop: 6→7→8→10)
...
| `lastReviewCommit` | string/null | HEAD hash at last review completion (Phase 7/8), used as "since last review" diff base option |
...
- **Files**: `ai-agent/skills/run-quality-checks/SKILL.md`, `phases/6-quality-checks.md`, `ai-agent/skills/run-quality-checks/output-template.md`
